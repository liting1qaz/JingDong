...
let goodPageSize = $.getdata('jdUnsubscribePageSize') || 380;// 运行一次取消全部已关注的商品。数字0表示不取关任何收藏商品
let shopPageSize = $.getdata('jdUnsubscribeShopPageSize') || 380;// 运行一次取消全部已关注的店铺。数字0表示不取关任何店铺
let stopGoods = $.getdata('jdUnsubscribeStopGoods') || '';//遇到此商品不再进行取关
let stopShop = $.getdata('jdUnsubscribeStopShop') || '';//遇到此店铺不再进行取关
